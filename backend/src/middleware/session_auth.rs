...
pub async fn session_auth_middleware(
    ...
    let jwt_claims = middleware.auth_service.validate_jwt_token(&token)
    ...
    if let Some(session_manager) = &middleware.session_manager {
        ...
        let session_count = session_manager.get_user_session_count(user_id).await
    ...
    let user = middleware.auth_service.get_user_by_id(user_id).await
...
fn extract_jwt_from_cookies(request: &Request) -> Result<String, AppError> {
    ...
    Err(AppError::AuthenticationError("No authentication token found".to_string()))
}
...
pub async fn enhanced_logout(
    ...
    if let Some(session_manager) = session_manager {
        session_manager.invalidate_user_sessions(user_id).await
            .map_err(|e| AppError::InternalServerError(format!("Session invalidation error: {}", e)))?;
    }
        ...
        if let (Some(request_ip), Some(stored_ip)) = (ip_address, &stored_session.ip_address) {
            if request_ip != stored_ip.as_ref() {
                tracing::warn!(
                    ...
                    stored_session.user_id, request_ip, stored_ip
                );
        ...
        if let (Some(request_ua), Some(stored_ua)) = (user_agent, &stored_session.user_agent) {
            if request_ua != stored_ua.as_ref() {
                tracing::debug!(
                    ...
                    stored_session.user_id, request_ua, stored_ua
                );
