...
mod tests {
    ...
    fn test_refresh_token_generation() {
        ...
        assert_eq!(
            token1.len(),
            64,
            "Refresh token should be 64 characters long"
        );
        assert_eq!(
            token2.len(),
            64,
        ...
        assert!(
            token2.chars().all(|c| c.is_ascii_alphanumeric()),
            "Token should only contain alphanumeric characters"
        );
    }
    ...
    fn test_jwt_config_security_requirements() {
        ...
        assert_eq!(
            config.current_secret, valid_secret,
            "Secret should be stored correctly"
        );
    }
    ...
    fn test_authentication_constants() {
        ...
        assert_eq!(
            expected_jwt_expiry_minutes, 15,
        ...
        assert_eq!(
            refresh_token.len(),
            64,
            "Refresh tokens should be 64 characters for security"
        );
    }
