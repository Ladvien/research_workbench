...
mod jwt_security_integration_tests {
    use crate::tests::jwt_test_utils::{
        create_expired_jwt_token, create_invalid_jwt_token, create_test_admin_jwt_token,
        create_test_jwt_token, validate_test_jwt_token, JwtTestScenario,
    };
    use axum::{
        body::Body,
    ...
    fn test_real_jwt_token_generation() {
        ...
        assert_ne!(
            ...
            "SECURITY VULNERABILITY: Still using mock JWT token!"
        );
    ...
    fn test_jwt_token_validation() {
        ...
        let is_valid = validate_test_jwt_token(&token).expect("Failed to validate JWT token");
    ...
    fn test_admin_jwt_token_generation() {
        ...
        assert_ne!(
            admin_token, "mock.jwt.token",
            "Admin JWT should not be mock token"
        );

        // Validate admin token
        let is_valid =
            validate_test_jwt_token(&admin_token).expect("Failed to validate admin JWT token");
        ...
        assert_ne!(
            ...
            "Admin and regular JWT tokens should be different"
        );
    }
    ...
    fn test_expired_jwt_token_validation() {
        ...
        assert_ne!(
            expired_token, "mock.jwt.token",
            "Expired JWT should not be mock token"
        );
        ...
        assert_eq!(
            parts.len(),
            3,
            "Expired JWT token should have correct structure"
        );

        // Expired token should fail validation
        let is_valid =
            validate_test_jwt_token(&expired_token).expect("Failed to validate expired JWT token");
    ...
    fn test_invalid_jwt_token_validation() {
        ...
        let is_valid =
            validate_test_jwt_token(&invalid_token).expect("Failed to validate invalid JWT token");
    ...
    fn test_jwt_test_scenarios_comprehensive() {
        ...
        for scenario in &scenarios {
            let token = scenario.generate_token().expect(&format!(
                "Failed to generate token for scenario: {:?}",
                scenario
            ));
            ...
            assert_ne!(
                token, "mock.jwt.token",
                "Scenario {:?} should not return mock token",
                scenario
            );
            ...
            assert!(
                expected_status == 200 || expected_status == 401,
                "Scenario {:?} should return valid HTTP status",
                scenario
            );
        }
    }
    ...
    fn test_jwt_security_properties() {
        ...
        assert!(
            token.len() > 50,
            "JWT token should be sufficiently long for security"
        );

        // Token should not contain obvious test strings
        assert!(
            !token.contains("test"),
            "JWT token should not contain obvious test strings"
        );
        assert!(
            !token.contains("mock"),
            "JWT token should not contain mock indicators"
        );
        assert!(
            !token.contains("fake"),
            "JWT token should not contain fake indicators"
        );
        ...
        assert_ne!(
            token, token2,
            "JWT tokens should be unique due to timestamps"
        );
    }
    ...
    fn test_jwt_token_claims_extraction() {
        ...
        assert_eq!(
            ...
            "Key version should be 1 for test config"
        );
    }

    /// Helper function to create a test app for HTTP testing
    async fn create_test_app() -> Router {
        Router::new().route("/api/protected", get(mock_protected_handler))
    }
    ...
    fn test_no_mock_tokens_security_regression() {
        ...
        let scenarios = [JwtTestScenario::ValidUser, JwtTestScenario::ValidAdmin];
        ...
        for scenario in &scenarios {
            ...
            assert_ne!(
                token, "mock.jwt.token",
                "SECURITY REGRESSION: Mock token detected in scenario {:?}",
                scenario
            );
            assert_ne!(
                token, "test.jwt.token",
                "SECURITY REGRESSION: Test mock token detected in scenario {:?}",
                scenario
            );
            assert_ne!(
                token, "fake.jwt.token",
                "SECURITY REGRESSION: Fake mock token detected in scenario {:?}",
                scenario
            );
        }
    }
