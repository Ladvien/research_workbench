...
use workbench_server::{
    config::{AppConfig, JwtConfig, RateLimitConfig, CookieSecurityConfig},
    error::AppError,
    models::{LoginRequest, RegisterRequest, UserResponse, RefreshTokenRequest},
...
impl MockAuthService {
    ...
    fn login_user(&self, email: &str, password: &str) -> Result<(UserResponse, String, String), AppError> {
    ...
    fn validate_token(&self, token: &str) -> Result<UserResponse, AppError> {
        ...
        let email = tokens.get(token).ok_or_else(|| {
    ...
    fn refresh_access_token(&self, refresh_token: &str) -> Result<(UserResponse, String, String), AppError> {
        let refresh_tokens = self.refresh_tokens.lock().expect("Test assertion failed");
        let users = self.users.lock().expect("Test assertion failed");
        
        let email = refresh_tokens.get(refresh_token).ok_or_else(|| {
...
async fn test_complete_authentication_flow() {
    ...
    let user = mock_auth.register_user(email, username).expect("Registration should succeed");
    ...
    assert!(invalid_token_result.is_err(), "Invalid token validation should fail");
    ...
    assert!(old_token_validation.is_ok(), "Old access token should still work in this mock");
...
async fn test_jwt_token_properties() {
    ...
    assert!(short_secret_result.is_err(), "Short JWT secret should be rejected");
}
...
async fn test_authentication_error_handling() {
    ...
    assert!(login_result.is_err(), "Login with non-existent user should fail");
    
    // Test token validation with invalid token
    let validation_result = mock_auth.validate_token("completely_invalid_token");
    assert!(validation_result.is_err(), "Invalid token validation should fail");
...
async fn test_user_data_integrity() {
    ...
    assert_eq!(user.email, login_user.email, "User email should be consistent");
    assert_eq!(user.username, login_user.username, "User username should be consistent");
    
    // Validate token returns same user
    let token_user = mock_auth.validate_token(&access_token).expect("Token validation should succeed");
    assert_eq!(user.id, token_user.id, "Token should return same user ID");
    assert_eq!(user.email, token_user.email, "Token should return same user email");
    assert_eq!(user.username, token_user.username, "Token should return same user username");
}
...
async fn test_authentication_request_validation() {
    ...
    assert!(valid_login.validate().is_ok(), "Valid login request should pass validation");
    ...
    assert!(invalid_email_login.validate().is_err(), "Invalid email should fail validation");
    ...
    assert!(valid_register.validate().is_ok(), "Valid registration request should pass validation");
    ...
    assert!(invalid_username_register.validate().is_err(), "Short username should fail validation");
    
    // Test valid refresh token request
    let valid_refresh = RefreshTokenRequest {
        refresh_token: "valid_refresh_token_string".to_string(),
    };
    assert!(valid_refresh.validate().is_ok(), "Valid refresh token request should pass validation");
    
    // Test empty refresh token
    let empty_refresh = RefreshTokenRequest {
        refresh_token: "".to_string(),
    };
    assert!(empty_refresh.validate().is_err(), "Empty refresh token should fail validation");
}
