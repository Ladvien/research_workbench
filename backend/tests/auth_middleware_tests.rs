...
async fn test_jwt_token_validation_invalid_signature() {
    ...
    assert!(result.is_err(), "JWT token with wrong signature should fail validation");
}
...
async fn test_jwt_token_validation_malformed() {
    ...
    assert!(result.is_err(), "Malformed JWT token should fail validation");
}
...
async fn test_authentication_error_scenarios() {
    ...
    assert!(matches!(invalid_format_error, AppError::AuthenticationError(_)));
    ...
    assert!(matches!(user_not_found_error, AppError::AuthenticationError(_)));
}
...
async fn test_jwt_key_rotation_support() {
    ...
    jwt_config.add_previous_secret(0, old_secret.to_string()).expect("Should add previous secret");
    
    // Verify previous secret is stored
    assert_eq!(jwt_config.previous_secrets.len(), 1);
    assert_eq!(jwt_config.get_secret_for_version(0), Some(&old_secret.to_string()));
    assert_eq!(jwt_config.get_secret_for_version(1), Some(&jwt_config.current_secret));
    
    // Test secret rotation
    let new_secret = "new-secret-that-is-32-characters-long-for-security!";
    jwt_config.rotate_secret(new_secret.to_string()).expect("Should rotate secret");
