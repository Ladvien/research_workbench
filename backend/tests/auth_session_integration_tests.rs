...
async fn test_session_invalidation_on_logout() {
    ...
    let session_count_before = session_manager.get_user_session_count(user_id).await.unwrap();
    assert_eq!(session_count_before, 1, "Should have 1 session before logout");
    ...
    let session_count_after = session_manager.get_user_session_count(user_id).await.unwrap();
    assert_eq!(session_count_after, 0, "Should have 0 sessions after logout");
...
async fn test_redis_session_integration() {
    ...
    let store_result = session_manager.store_session(session_id, session_data.clone()).await;
    ...
    assert!(after_delete.is_none(), "Session should be deleted from Redis");
...
async fn test_session_security_validation() {
    ...
    let result = crate::middleware::session_auth::SessionSecurityValidator::validate_session_request(
        Some("192.168.1.100"),
        Some("Valid Agent"),
        &valid_session,
    );
    assert!(result.is_ok(), "Valid session should pass security validation");
    ...
    assert!(result.is_err(), "Expired session should fail security validation");
    ...
    assert!(result.is_err(), "Idle session should fail security validation");
...
async fn test_password_change_session_invalidation() {
    ...
    let count_before = session_manager.get_user_session_count(user_id).await.unwrap();
    assert_eq!(count_before, 4, "Should have 4 sessions before password change");

    // Simulate password change by invalidating all user sessions
    session_manager.invalidate_user_sessions(user_id).await.unwrap();

    // Verify all sessions are gone
    let count_after = session_manager.get_user_session_count(user_id).await.unwrap();
    assert_eq!(count_after, 0, "Should have 0 sessions after password change");
...
async fn test_session_performance() {
    ...
    assert!(creation_time.as_millis() < 10000, "Session creation should be reasonably fast");
    assert!(count_time.as_millis() < 100, "Session count should be very fast");
    assert!(session_count <= 50, "Should enforce session limits");

    println!("✅ Performance test: {} sessions handled in {:?}, count in {:?}", 
