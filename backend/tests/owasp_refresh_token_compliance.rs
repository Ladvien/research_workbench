...
async fn setup_owasp_compliant_auth_service() -> Result<AuthService, AppError> {
    let database_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://postgres:postgres@192.168.1.104:5432/workbench".to_string()
    });
    ...
    let jwt_config = JwtConfig {
        current_secret:
            "owasp_compliant_secret_key_for_testing_that_meets_minimum_length_requirements"
                .to_string(),
...
async fn test_owasp_requirement_1_short_lived_access_tokens() {
    ...
    let claims = auth_service
        .validate_jwt_token(&response.access_token)
        .unwrap();
    ...
    println!(
        "✅ OWASP Requirement 1: Access token expires in {} minutes (≤15 minutes)",
        token_lifetime_minutes
    );
}
...
async fn test_owasp_requirement_2_refresh_token_rotation() {
    ...
    let first_refresh = auth_service
        .refresh_access_token(&original_refresh_token)
        .await
        .unwrap();
    ...
    let second_refresh = auth_service
        .refresh_access_token(&rotated_refresh_token)
        .await;
    assert!(
...
async fn test_owasp_requirement_3_secure_token_storage() {
    ...
    // Store the token
    let _stored_token = refresh_repo
        .create_refresh_token(user_id, &raw_token)
        .await
        .unwrap();

    // Retrieve and verify it's hashed
    let found_token = refresh_repo
    ...
    assert_ne!(
        found_token.token_hash, raw_token,
    ...
    assert_eq!(
        found_token.token_hash, expected_hash,
...
async fn test_owasp_requirement_5_session_cleanup_on_security_events() {
    ...
    let password_change_result = auth_service
        .change_password(user_id, "SecurePassword123!", "NewSecurePassword456!")
        .await;
    assert!(
        password_change_result.is_ok(),
    ...
    println!(
        "✅ OWASP Requirement 5: Refresh tokens invalidated on security events (password change)"
    );
}
...
async fn test_complete_owasp_jwt_security_compliance() {
    ...
    let stored_token = refresh_repo
        .find_by_token_hash(&first_refresh.refresh_token)
        .await
        .unwrap()
        .unwrap();
    assert_ne!(
        stored_token.token_hash, first_refresh.refresh_token,
    ...
    let logout_result = auth_service
        .logout_with_refresh_token(&first_refresh.refresh_token)
        .await;
    assert!(logout_result.is_ok(), "Logout should succeed");

    let post_logout_test = auth_service
        .refresh_access_token(&first_refresh.refresh_token)
        .await;
