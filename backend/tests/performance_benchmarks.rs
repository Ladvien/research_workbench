...
mod performance_benchmarks {
    ...
    async fn benchmark_database_conversation_query() {
        ...
        app_state.dal.users().create(test_user).await.unwrap();
        ...
        app_state.dal.conversations().create(test_conversation).await.unwrap();
    ...
    async fn benchmark_concurrent_database_queries() {
        ...
        for i in 0..10 {
            ...
            app_state.dal.conversations().create(conversation.clone()).await.unwrap();
    ...
    fn generate_title_uncached(content: &str) -> String {
        let clean_content = content.trim().replace(/\s+/g, " ");
        if clean_content.len() <= 30 {
            return clean_content;
        }
        
        let first_sentence = clean_content.split(/[.!?]/).next().unwrap_or("");
    ...
    fn generate_title_cached(content: &str, cache: &mut std::collections::HashMap<String, String>) -> String {
