...
async fn test_refresh_token_rotation() {
    ...
    assert_ne!(
        original_refresh_token, new_refresh_token,
...
async fn test_refresh_token_hashing() {
    ...
    assert_eq!(
        found_token.token_hash, token_hash,
...
async fn test_refresh_token_cleanup_on_password_change() {
    ...
    let change_result = auth_service
        .change_password(user_id, "SecureTestPassword123!", "NewSecurePassword456!")
        .await;
    ...
    assert!(
        refresh_after_change.is_err(),
        "Refresh token should be invalidated after password change"
    );
}
...
async fn test_owasp_security_requirements() {
    ...
    assert!(
        token_lifetime <= 16 * 60,
        "Access token should expire within 16 minutes (15 + 1 for tolerance)"
    );
    ...
    assert_ne!(
        ...
        "Refresh token must be hashed in storage"
    );
