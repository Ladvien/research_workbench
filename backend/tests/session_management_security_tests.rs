...
mod session_security_tests {
    ...
    async fn test_session_creation_and_retrieval() {
        ...
        // Store session
        let result = session_manager.store_session(session_id, session_data.clone()).await;
        ...
        assert_eq!(retrieved_data.user_agent, Some("Mozilla/5.0 Test Agent".into()));
    }
    ...
    async fn test_session_invalidation_on_logout() {
        ...
        assert!(before_delete.is_some(), "Session should exist before deletion");
        ...
        assert!(after_delete.is_none(), "Session should not exist after deletion");
    }
    ...
    async fn test_concurrent_session_limits() {
        ...
        let session_count = session_manager.get_user_session_count(user_id).await.unwrap();
    ...
    async fn test_user_session_invalidation() {
        ...
        let before_count = session_manager.get_user_session_count(user_id).await.unwrap();
        assert_eq!(before_count, 3, "Should have 3 sessions before invalidation");
        ...
        assert!(invalidate_result.is_ok(), "User session invalidation should succeed");

        // Verify all sessions are gone
        let after_count = session_manager.get_user_session_count(user_id).await.unwrap();
    ...
    async fn test_expired_session_cleanup() {
        ...
        assert!(cleaned_count > 0, "Should have cleaned at least 1 expired session");
    }
    ...
    async fn test_redis_security_validation() {
        ...
        let result = session_manager.store_session("test_session", session_data).await;
    ...
    async fn test_session_data_integrity() {
        ...
        session_manager.store_session(session_id, session_data.clone()).await.unwrap();
        
        let retrieved = session_manager.get_session(session_id).await.unwrap().unwrap();
        assert_eq!(retrieved.user_id, user_id);
        assert_eq!(retrieved.ip_address, Some("::1".into()));
        assert_eq!(retrieved.user_agent, Some("Mozilla/5.0 (Test) 测试 🔒".into()));
    }
    ...
    async fn test_user_session_isolation() {
        ...
        for i in 1..=3 {
            ...
            session_manager.store_session(&format!("user1_session_{}", i), session_data1).await.unwrap();
            session_manager.store_session(&format!("user2_session_{}", i), session_data2).await.unwrap();
        }

        // Check session counts
        let user1_count = session_manager.get_user_session_count(user1_id).await.unwrap();
        let user2_count = session_manager.get_user_session_count(user2_id).await.unwrap();
        ...
        session_manager.invalidate_user_sessions(user1_id).await.unwrap();

        // Check that only user1's sessions are gone
        let user1_count_after = session_manager.get_user_session_count(user1_id).await.unwrap();
        let user2_count_after = session_manager.get_user_session_count(user2_id).await.unwrap();
    ...
    async fn test_session_performance() {
        ...
        assert!(creation_time.as_millis() < 5000, "Session creation should be fast"); // 5 seconds max
        assert!(count_time.as_millis() < 100, "Session count should be very fast"); // 100ms max
        
        println!("✅ Performance test: {} sessions created in {:?}, count retrieved in {:?}", 
...
mod hybrid_auth_conflict_tests {
    ...
    async fn test_hybrid_jwt_session_conflict() {
        ...
        // Session exists
        let session_exists = session_manager.get_session(session_id).await.unwrap().is_some();
        ...
        assert!(session_after_logout.is_none(), "Session should be deleted after logout");
    ...
    async fn test_proposed_session_jwt_binding() {
        ...
        assert!(auth_is_valid, "Authentication should require both JWT and session");
        
        // After logout, session is deleted
        session_manager.delete_session(session_id).await.unwrap();
        
        // Now authentication fails even with valid JWT
        let session_exists_after_logout = session_manager.get_session(session_id).await.unwrap().is_some();
        let auth_after_logout = jwt_is_valid && session_exists_after_logout;
        
        assert!(!auth_after_logout, "Authentication should fail when session is deleted");
